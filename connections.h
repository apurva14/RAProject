...
//#include <asm/div64.h>	// for 64 bit division using do_div(x,y)
#include "wireless.h" // for sleep_device() and wake_device() functions.
...
#define PACKET_COUNT_THRESHOLD	40
<<<<<<< HEAD
#define DATA_THRESHOLD			30000		// 50 KBs
=======
#define DATA_THRESHOLD			40000		// 50 KBs
...
#define BUFFER_TIME_FOR_EXTRA_PACKETS 2
...
struct constate {
	...
	u_int8_t flag; /* Keeps care of the ACK to ensure that they do not stay more than one at a time
					* Can be 0 (No incoming), 1(One incoming) or 2(two incoming)
					*/
	u_int8_t flag_s;
	u_int8_t flag_dupack;
	u_int8_t count;
	u_int8_t recievedFullBurst_flag;
	u_int8_t chokeTimerInitiated_flag;

	//For sending duplicate ACK
	struct tcphdr* dup_tcph;
	struct iphdr* dup_iph;
	struct sk_buff* dup_my_skb;
	struct sk_buff* dup_my_skb1;
<<<<<<< HEAD
	struct tcphdr* dup_tcph1;
	struct iphdr* dup_iph1;
=======
>>>>>>> b4758aaf1cc416cece8b165b7c7042cf199cf643
...
//static inline void send_Duplicate_ACK(struct constate* connection);
//static inline void send_Duplicate_ACK(struct sk_buff *skb, u32 seq, u32 ack,
//				    u32 win, u32 ts, int oif,
//				    int reply_flags);
//static inline void ip_send_reply_dup(struct sock *sk, struct sk_buff *skb, struct ip_reply_arg *arg,unsigned int len);
...
static inline void add_connection(struct constate** arg_con,
		struct iphdr* arg_ip, struct tcphdr* arg_tcp, u_int32_t arg_con_id,
		u_int8_t state) {
	...
	if (*arg_con == NULL) {
		...
		temp->count = 0;
		temp->recievedFullBurst_flag = 0;
		temp->chokeTimerInitiated_flag = 0;
		temp->flag_s = 0;
		//for duplicate ACK
		temp->dup_iph = NULL;
<<<<<<< HEAD
		temp->dup_iph1 = NULL;
		temp->dup_my_skb=NULL;
		temp->dup_my_skb1=NULL;
		temp->dup_tcph=NULL;
		temp->dup_tcph1=NULL;
=======
		temp->dup_my_skb=NULL;
...
<<<<<<< HEAD
		node->dup_iph1 = NULL;
		node->dup_my_skb=NULL;
		node->dup_my_skb1=NULL;
		node->dup_tcph=NULL;
		node->dup_tcph1=NULL;
=======
		node->dup_my_skb=NULL;
...
//		node->count = 0;
//		node->flag_s = 0;
//		node->recievedFullBurst_flag = 0;
//		node->chokeTimerInitiated_flag = 0;
		node->count = 0;
		node->flag_s = 0;
		node->recievedFullBurst_flag = 0;
		node->chokeTimerInitiated_flag = 0;
...
static inline void update_rtt(struct constate* node,
	...
	if (node != NULL && conskb != NULL) {
		...
		//printk ("DEBUG:%d ConId: %u Before Update RTT = %d \n", jiffies, node->connection_id, node->rcv_rtt);
		update_tcpi_rtt(node, conskb);
		//printk ("DEBUG:%d ConId: %u After Update RTT = %d \n", jiffies, node->connection_id, node->rcv_rtt);
	}
...
static inline u_int8_t refresh_rtt(struct constate* node,
	...
	if (tp) {
		...
		//printk ("DEBUG:%d ConId: %u Refresh RTT = %d \n", jiffies, node->connection_id, node->rcv_rtt);
		return 0;
	}
...
static inline u_int8_t update_tcpi_rtt(struct constate* node,
	...
	if (sk != NULL ){
		...
		if (&temp_tcp_info != NULL){
			...
			//printk ("DEBUG:%d ConId: %u Update TCPI RTT: rtt = %d tcpi_rcv_rtt = %d rtt_var = %d \n", jiffies, node->connection_id, node->rtt, node->tcpi_rcv_rtt, node->rtt_var);
		}
...
static inline u_int8_t update_window_scale_value(struct constate* node,
	...
	if (sk != NULL ){
		...
		if (&temp_tcp_info != NULL){
			...
			//printk ("DEBUG:%d ConId: %u Window Scale = %d \n", jiffies, node->connection_id, node->window_scale);
		}
...
static inline u_int8_t update_tcpi_rcv_mss(struct constate* node,
	...
	if (sk != NULL ){
		...
		if (&temp_tcp_info != NULL){
			 ...
			 */
			//printk ("DEBUG:%d ConId: %u TCPI_RCV_MSS This function allocates a default MSS Value i.e. 1452 \n", jiffies, node->connection_id, node->tcpi_rcv_mss);
			//node->tcpi_rcv_mss = temp_tcp_info.tcpi_rcv_mss;
			node->tcpi_rcv_mss = DEFAULT_MSS_VALUE;
			printk ("DEBUG:%d ConId: %u TCPI_RCV_MSS = %d \n", jiffies, node->connection_id, node->tcpi_rcv_mss);
...
//		connection->flow_rate_normal = temp_fr;
		...
		connection->flow_rate_normal = ((float)connection->temp_data_arrived/ time_period )  * 1000;
		//printk ("DEBUG:%d ConId: %u Conn_Start_Time= %d frn= %d \n", jiffies, connection->connection_id, connection->connection_start_time, connection->flow_rate_normal);
//		printk("ConId : %u time_period: %d, flow_rate_normal: %d\n",
//				connection->connection_id, time_period, connection->flow_rate_normal);
...
//		connection->flow_rate_td = temp_fr;
		printk ("DEBUG:%d ConId: %u TimePeriod= %d data_arrived_td= %d \n", jiffies, connection->connection_id, time_period, connection->data_arrived_td);
		connection->flow_rate_td = ((float)connection->data_arrived_td	/ time_period)  * 1000;
		printk ("DEBUG:%d ConId: %u Td_Start_Time= %d frtd= %d \n", jiffies, connection->connection_id, connection->td_start_time, connection->flow_rate_td);
//		printk("ConId : %u burst_time: %d, flow_rate_td: %d\n",
//				connection->connection_id, time_period, connection->flow_rate_td);
...
//		connection->flow_rate_psmt = temp_fr;
		...
		connection->flow_rate_psmt = ((float)connection->temp_data_arrived	/ time_period )  * 1000;
...
static inline void modify_sleep_timer(struct constate *connection, const unsigned int wait_time){
	...
	printk ("DEBUG:%d ConId: %u Modify Sleep timer expires in %d wait time %u \n", jiffies, connection->connection_id, expires, wait_time);
}


static inline u_int32_t calculate_window_size(u_int32_t flowrate_normal , u_int32_t rtt){
	//printk ("DEBUG:%d ConId: %u Cal_Win_Size \n", jiffies, connection->connection_id);
	return ((float)flowrate_normal*rtt)/1000;
...
static inline void reduce_window_size(struct constate * connection){
	...
	if ( (signed int)( connection->psmt_window_size - connection->tcpi_rcv_mss)	>= ( signed int)connection->psmt_window_size_min) { // casting to int to get signed values
		...
		printk ("DEBUG:%d ConId: %u Red_Win_Size \n", jiffies, connection->connection_id);
	}
...
static inline void scheduler(struct constate * node, int idle_state, char * log_message){
	...
	printk ("DEBUG:%d ConId: %u Scheduler \n", jiffies, node->connection_id);
	...
	case IDLE:
		 ...
		 */
		printk ("DEBUG:%d ConId: %u Scheduler Start of IDLE case \n", jiffies, node->connection_id);
		if (node->idle_state == NOT_IDLE) {
...
//					}
			...
			if (is_wnic_sleep()) {
				if (sleep_wnic_check(&all_connections_head, node->connection_id)) {
					printk ("DEBUG:%d ConId: %u Scheduler IDLE \n", jiffies, node->connection_id);
					wni_control(IDLE);
					printk("SLEEP : %s\n", log_message);
				} else {
					printk("< no time to sleep >\n");
	...
	case NOT_IDLE:
		 ...
		 */
		printk ("DEBUG:%d ConId: %u Scheduler Start of NOTIDLE Case\n", jiffies, node->connection_id);
...
//				}
			...
			printk ("DEBUG:%d ConId: %u Scheduler NOTIDLE \n", jiffies, node->connection_id);
...
static inline void sleep_timer_function(unsigned long data) {
	 ...
	 */
	printk ("DEBUG:%d ConId: %u SleepTimer \n", jiffies, connection->connection_id);
...
static inline void wake_timer_function(unsigned long data) {
	 ...
	 */
	printk ("DEBUG:%d ConId: %u WakeTimer \n", jiffies, connection->connection_id);
...
//static inline void send_Duplicate_ACK(struct constate * connection) {
//
//	struct tcphdr* d_tcph;
//	struct iphdr* d_iph;
//	struct sk_buff* d_my_skb;
//	struct socket *sk;
//
//	//d_iph = connection->dup_iph;
//	d_my_skb = connection->dup_my_skb;
//	d_tcph = tcp_hdr(d_my_skb);
//	d_iph = ip_hdr(d_my_skb);
//	//d_tcph = connection->dup_tcph;
//	sk = d_my_skb->sk;
//	//timeval stamp1;
//	//struct tcp_opt *tp = sk->tp_pinfo
//	//struct tcp_tw_bucket tw;
//	//tw = kmalloc(sizeof(struct tcp_tw_bucket), GFP_KERNEL);
//	//tw = (struct tcp_tw_bucket *)sk;
//	//tcp_v4_send_ack(skb, tw->snd_nxt, tw->rcv_nxt, tw->rcv_wnd>>tw->rcv_wscale, tw->ts_recent);
//	//struct tcphdr *th = d_my_skb->h.th;
//	struct tcphdr *th = tcp_hdr(d_my_skb);
//	struct {
//	struct tcphdr th;
//	unsigned long tsopt[3];
//	} rep;
...
//	long ts;
//	//unsigned long ts;
//	unsigned long seq; unsigned long ack; unsigned long win;
//	//ts = (u32)
//	//long t;
//	//ts = tw->ts_recent_stamp;
//	seq = d_tcph->seq;
//	ack = d_tcph->ack;
//	win = d_tcph->window;
//	if (ts) {
...
//	rep.tsopt[2] = htonl(tcp_time_stamp);
//	//rep.tsopt[2] = htonl(ts);
//	arg.iov[0].iov_len = sizeof(rep);
//	}
//	/* Swap the send and the receive with a little tweaking. */
//	rep.th.dest = th->dest;
//	rep.th.source = th->source;
...
//
//
//	arg.csum = csum_tcpudp_nofold(d_iph->daddr, d_iph->saddr, arg.iov[0].iov_len, IPPROTO_TCP, 0);
//	arg.csumoffset = offsetof(struct tcphdr, check) / 2;
//	printk ("DEBUG:%d ConId: %u Send_DuplicateAck seq = %lu ack_seq = %lu win = %lu \n", jiffies, connection->connection_id, seq, ack, win);
//
...
//        u32 daddr;
//         struct rtable *rt = (struct rtable*)d_tcph->dest;
//        if (ip_options_echo(&replyopts.opt, d_my_skb))
//                 return;
//
...
//          */
//         bh_lock_sock(sk);
//         sk->protinfo.af_inet.tos = d_iph->tos;
//         sk->priority = d_tcph->urg;
//         sk->protocol = d_iph->protocol;
//         ip_build_xmit(sk, ip_reply_glue_bits, arg, len, &ipc, rt, MSG_DONTWAIT);
//         ip_build_xmit(sk, 0, arg, len, &ipc, rt, MSG_DONTWAIT);
//         bh_unlock_sock(sk);
//
//         ip_rt_put(rt);
// }
	...
	if (node != NULL) {
		...
		if (node->state == (SYNED | ACKED | ESTABLISHED)) {
			...
			if (node->choke_state == NO_OP) {
				printk ("DEBUG:%d ConId: %u TimerFN State=EST ChokeState NO_OP \n", jiffies, node->connection_id);
				node->choke_state = CALC_FLOWRATE;

				mod_timer(&node->timer, jiffies
						+ msecs_to_jiffies(calc_flowrate_wait));
				printk ("DEBUG:%d ConId: %u TimerFN Called Time again after 5000msecs i.e. %d in jiffies \n", jiffies, node->connection_id, msecs_to_jiffies(calc_flowrate_wait));
				node->flow_rate_normal = 0;
				node->temp_data_arrived = 0;
				node->connection_start_time = jiffies;
				printk ("DEBUG:%d ConId: %u TimerFn con_start_time = jiffy frn =0 temp_data_arrived=0  \n", jiffies, node->connection_id);

			} else if (node->choke_state == CALC_FLOWRATE) {
				printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE \n", jiffies, node->connection_id);
<<<<<<< HEAD
				printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE frn = %d\n", jiffies, node->connection_id, node->flow_rate_normal);
				printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE total packet count = %d\n", jiffies, node->connection_id, node->total_packet_count);
				printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE temp data arrived = %d\n", jiffies, node->connection_id, node->temp_data_arrived);
				if (node->total_packet_count >= PACKET_COUNT_THRESHOLD && node->temp_data_arrived >= DATA_THRESHOLD) {
=======
...
>>>>>>> b4758aaf1cc416cece8b165b7c7042cf199cf643
					...
					printk ("DEBUG:%d ConId: %u TimerFn td_start_time = jiffy \n", jiffies, node->connection_id);
				}
				else {
					printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE frn = %d\n", jiffies, node->connection_id, node->flow_rate_normal);
					printk ("DEBUG:%d ConId: %u TimerFN ChokeState CALC_FLOWRATE total packet count = %d\n", jiffies, node->connection_id, node->total_packet_count);
			 ...
			 */
			node->choke_state = NO_OP;
			printk ("DEBUG:%d ConId: %u TimerFn state = TD choke_state = NO_OP \n", jiffies, node->connection_id);
...
//				}
				...
				printk ("DEBUG:%d ConId: %u Throttle Detected FR_N = %d  FR_TD = %d\n \n", jiffies, node->connection_id,node->flow_rate_normal, node->flow_rate_td);
				printk ("DEBUG:%d ConId: %u TimerFn Calling cal_win_size psmt_win_size= %d rtt in msecs= %d \n", jiffies, node->connection_id, node->psmt_window_size, node->rtt/1000);

			} else {
				printk ("DEBUG:%d ConId: %u Throttle Not Detected FR_N = %d  FR_TD = %d\n psmt_window_size = %d \n", jiffies, node->connection_id,node->flow_rate_normal, node->flow_rate_td, node->psmt_window_size);
			...
			case INITIAL_WAIT:
				 ...
				 * Preparing for PSM State..
				 */
				printk ("DEBUG:%d ConId: %u Timer: InitialChokeWait \n", jiffies, node->connection_id);
				node->temp_data_arrived = 0; // reset the data arrived for calculation of flow rate
				node->psmt_start_time = jiffies; // mark the time when we enter psmt state
				node->flag_dupack = 1;
				if (EMULATE_WNIC){
				...
				printk ("DEBUG:%d ConId: %u Timer: InitialChokeWait count = %d flag_s = %d\n", jiffies, node->connection_id, node->count, node->flag_s);
...
//				printk(" ( |/ ) ADVERTISE_WINDOW_SIZE -> %s\n",	get_psmt_state_name(node->psmt_state));
				printk ("DEBUG:%d ConId: %u Timer: AWS Calling send_duplicate_ack\n", jiffies, node->connection_id);
//				struct sk_buff *skb; u32 seq; u32 ack;
//								    u32 win; u32 ts; int oif;
//								    int reply_flags;
				int ret = send_Duplicate_ACK(node);
				node->flag_dupack = 0;
				node->flag_s = 1;
//				modify_sleep_timer(node,(node->rtt-((node->rtt_var*AGGRESSION_VALUE_N)/AGGRESSION_VALUE_D) - TRANSITION_TIME_SLEEP_TO_WAKE) / 1000);
				modify_sleep_timer(node,(node->rtt * 2 -((node->rtt_var*AGGRESSION_VALUE_N)/AGGRESSION_VALUE_D) - TRANSITION_TIME_SLEEP_TO_WAKE) / 1000);
				scheduler(node, IDLE, "INITIAL ADV SENT");
				node->psmt_state= CHOKE;
				printk ("DEBUG:%d ConId: %u Timer: INITIAL ADV SENT ret = %d \n", jiffies, node->connection_id, ret);
				break;
			...
			case RECEIVE_PACKETS:
				 ...
				 */
				printk ("DEBUG:%d ConId: %u Timer: CHKRP \n", jiffies, node->connection_id);
...
//				reduce_window_size(node);
				node->flag_dupack = 0;
				//node->chokeTimerInitiated_flag = 0;
//				if (node->recievedFullBurst_flag == 1) {
//					scheduler(node, NOT_IDLE, "Wake up - sending out a packet ..");
//					node->recievedFullBurst_flag = 0;
//				}
//				int i;
//				for(i =0;i<10000;i++) {
...
//				}
				node->recievedFullBurst_flag = 0;
				printk ("DEBUG:%d ConId: %u Timer: CHK RCV PKT Calling send_duplicate_ack\n", jiffies, node->connection_id);
				recalculate_window_size(node);
				node->data_arrived_burst = 0;
				ret = send_Duplicate_ACK(node);
//				modify_sleep_timer(node,(node->rtt-((node->rtt_var*AGGRESSION_VALUE_N)/AGGRESSION_VALUE_D) - TRANSITION_TIME_SLEEP_TO_WAKE) / 1000);
				modify_sleep_timer(node,(node->rtt * 2 -((node->rtt_var*AGGRESSION_VALUE_N)/AGGRESSION_VALUE_D) - TRANSITION_TIME_SLEEP_TO_WAKE) / 1000);
				scheduler(node, IDLE, "received full burst");
				node->psmt_state= CHOKE;
